...
static const Err_Entry Err_Table[ERR_CODE_COUNT] = {
    ...
    [ERR_STR_ESCAPE_OUT_OF_RANGE]       = { "ERR_STR_ESCAPE_OUT_OF_RANGE",       "escape sequence out of range for a %zu-byte character" },
    [ERR_STR_ESCAPE_HEX_EMPTY]          = { "ERR_STR_ESCAPE_HEX_EMPTY",          "\\x used with no following hex digits" },
